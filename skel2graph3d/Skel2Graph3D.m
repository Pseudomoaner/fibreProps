...
for i=1:cc2.NumObjects
    ...
    node(i).comy = y;
    node(i).comz = z;
    node(i).ptComx = mean(x);
    node(i).ptComy = mean(y);
    node(i).ptComz = mean(z);
    node(i).ep = 0;
...
for i=1:cc3.NumObjects
    ...
    node(ni).comy = y;
    node(ni).comz = z;
    node(ni).ptComx = mean(x);
    node(ni).ptComy = mean(y);
    node(ni).ptComz = mean(z);
    node(ni).ep = 1;
...
for i=1:length(node)
    ...
    node(i).ptComx = node(i).ptComx - 1;
    node(i).ptComy = node(i).ptComy - 1;
    node(i).ptComz = node(i).ptComz - 1;
end;
