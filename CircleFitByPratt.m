...
%There is an odd bug where the fitting doesn't work properly if the input
%data is symmetrical along both the x and y axes. As this implies the
%radius of the fitted circle must be infinite, set this to be the case and
%return.
xDiff = diff(XY(:,1)) + diff(flip(XY(:,1)));
yDiff = diff(XY(:,2)) + diff(flip(XY(:,2)));
if sum(xDiff == 0) == numel(xDiff) && sum(yDiff == 0) == numel(yDiff)
    Par = [NaN,NaN,inf];
    return
end
...
for iter=1:IterMax
    ...
    if (iter >= IterMax)
        ...
        Par = [NaN,NaN,inf]; %Doesn't converge because points lie along a single line
        return
    end
    if (xnew<0.)
